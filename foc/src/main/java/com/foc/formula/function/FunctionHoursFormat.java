...
public class FunctionHoursFormat extends Function {
	private static final String FUNCTION_NAME = "FORMAT_HOURS";
	...
	public synchronized Object compute(FFormulaNode formulaNode){
		...
		if(formulaNode != null){
			...
			if(childNodeHours != null){
				double hoursDouble = childNodeHours.getCalculatedValue_double();

				int hours   = (int) (hoursDouble);
				hoursDouble = hoursDouble - hours;
				hoursDouble = hoursDouble * 60;
				int minutes = (int) (hoursDouble);
				hoursDouble = hoursDouble - minutes;
				hoursDouble = hoursDouble * 60;
				int seconds = (int) (hoursDouble);
