...
## String/Text Field Quote Handling in Database

### Overview

NeoFOC automatically handles quote escaping for string/text fields when saving to the database. The framework converts double quotes to single quotes for certain database providers to prevent SQL syntax errors and ensure proper escaping.

### Implementation

**Location**: `neofoc/src/main/java/com/foc/property/FString.java`

### Quote Conversion by Database Provider

#### **Oracle & PostgreSQL**
```java
// Writing to DB (lines 139-144)
sqlStr = sqlStr.replaceAll("'", "''");    // Single quote → doubled (SQL escaping)
sqlStr = sqlStr.replaceAll("\"", "''");   // Double quote → two single quotes
sqlStr = "'" + sqlStr + "'";              // Wrap with single quotes

// Reading from DB (line 102)
str = str.replace("''", "\"");            // Convert back: '' → "
```

#### **MySQL**
```java
// Writing to DB (lines 148-150)
sqlStr = sqlStr.replaceAll("\"", "''");   // Double quote → two single quotes
sqlStr = "\"" + sqlStr + "\"";            // Wrap with double quotes
...
#### **MSSQL**
```java
// Writing to DB (lines 131-138)
sqlStr = sqlStr.replace("'", "''");       // Single quote → doubled (SQL escaping)
sqlStr = "N'" + sqlStr + "'";             // Wrap with Unicode marker
// Note: Double quotes are NOT converted for MSSQL
```

#### **H2**
```java
// Writing to DB (lines 145-147)
sqlStr = sqlStr.replaceAll("'", "''");    // Single quote → doubled (SQL escaping)
sqlStr = "'" + sqlStr + "'";              // Wrap with single quotes
// Note: Double quotes are NOT converted for H2
```

### Example Flow

**Original User Input**: `Hello "World"`

**Saved to Oracle/Postgres/MySQL**:
- SQL statement contains: `Hello ''World''` (double quotes converted to two single quotes)

**Read from Database**:
- Converted back to: `Hello "World"` (original value restored)

### Important Notes

1. **Round-Trip Consistency**: The conversion is symmetrical - what you save is what you get back
2. **SQL Injection Protection**: This is part of the SQL escaping mechanism to prevent SQL injection attacks
3. **Provider-Specific**: Only Oracle, PostgreSQL, and MySQL convert double quotes; MSSQL and H2 preserve them
4. **Compression Bypass**: If `isCompress()` is true, the string is compressed and no quote conversion happens
5. **Automatic**: This happens transparently - no manual escaping required in your code

### Related Files

- `neofoc/src/main/java/com/foc/property/FString.java:122-154` - Quote conversion logic
- `neofoc/src/main/java/com/foc/db/DBManager.java` - Database provider constants
- `neofoc/src/main/java/com/foc/desc/field/FStringField.java` - String field definition
