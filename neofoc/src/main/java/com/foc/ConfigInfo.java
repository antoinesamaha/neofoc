...
public class ConfigInfo {
    ...
    private static int passwordEncryptionMethod = FocUserDesc.PASSWORD_ENCRYPTION_METHOD_1;
    ...
    private static IApplicationYmlConfig applicationYmlConfig;

    public static String getProperty(String key) {
        String val = null;
        if(applicationYmlConfig != null) {
            val = applicationYmlConfig.getProperty("neofoc."+key);
        } else {
            val = Globals.getApp() != null ? Globals.getApp().getCommandLineArgument(key) : null;
...
//    }
        ...
        if (!loaded) {
            loaded = true;
            try {
                //In Spring boot application.yml is used
                if (applicationYmlConfig == null) {
    ...
    public static void setApplicationYmlConfig(IApplicationYmlConfig appYmlConfig) {
        applicationYmlConfig = appYmlConfig;
    }

    public interface IApplicationYmlConfig {
